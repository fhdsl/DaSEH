...
files = files[ files != "contact.Rmd" ]
files = files[ grepl("modules", files)]
files = files[ files != "modules/R_files/help.Rmd" ]
...
if (!dir.exists("modules/R_files")) {
  dir.create("modules/R_files")
}

all_rmd = all_rmd[ all_rmd != "./modules/R_files/help.Rmd" ]
all_rmd = all_rmd[ all_rmd != "./modules/R_files/Wastewater.Rmd" ]
...
write_rds(all_func, file = "resources/raw_all_func_forpuzzles.rds")
#### if copying files
