...
For that purpose, we will use influenza hospitalization rates from 2009 to 2025! You can read more about how this data is collected here:


### 3.1 Download the data:
Start by downloading the data
The data is located here:
http://daseh.org/data/FluSurveillance_2009_2025.csv

Hint: You will need to skip the first two rows of the data, use the argument `skip=` in the 
...
1. Use the `clean_names` command from the `janitor` package to clean up the column names.
2. Use the commands `filter` and `select` to obtain a data set with data for the "overall" value for the  age group, race and sex categories, and keeping only the columns "YEAR...3","YEAR...4", "WEEK", "VIRUS TYPE CATEGORY", "CUMULATIVE RATE" and "WEEKLY RATE".
3. Rename the columns `year_3` and `year_4` to more meaningful names: `flu_season` and `Year`  respectively. To avoid confusion with the `week` function in `lubridate`, rename the `week` column to `num_week`.

4. Check the types of all columns. Transform the columns that are meant to be numeric to numeric using the function `as.numeric()`.
...
fludat2 <- fludat2 %>% select(year_3,year_4,week, virus_type_category,
                        ...
                        rename("flu_season" = year_3, "Year" = year_4,
                               "num_week" = week) %>% mutate("weekly_rate" = as.numeric(weekly_rate))
```
...
Use `lubridate`'s arithmetic to do this. Create a new column called `week_date` by adding the beginning of the year given in the column `Year` (January 1st, Year) and the number of weeks given in the column `week`. 
...
```{r 3.4response}
df15_25 <- fludat2 %>% filter(flu_season %in% c("2014-15", "2024-25") | 
                           between(Year,2015, 2025))
p <- ggplot(df15_25, aes(x=week_date, y=as.numeric(weekly_rate), color= virus_type_category)) + geom_line()
p
```
...
1. What can you say about the 2020-2021 flu season? Do we have data for this season? Why? Is this data missing at random?
Hint: you can look at the definition of "missing at random" in the *Data Cleaning* module.
...
#2
covid19_min <- df15_25 %>% filter(flu_season == "2020-21", is.na(weekly_rate)) %>% pull(week_date) %>% min(na.rm = TRUE)
#Data stopped being collected on 2020-10-06.

#3 
covid19_max <- df15_25 %>% filter(flu_season == "2020-21", is.na(weekly_rate)) %>% pull(week_date) %>% max(na.rm = TRUE)
#The last week of data not collected was 2021-04-29.
...
### 3.6
We are going to focus now on comparing different influenza seasons. To do this, we will compare the overall mean weekly rate of all seasons to the the seasons with the minimum and maximum peak hospitalization weekly rates.
...
compare_df <- df15_25 %>% group_by(flu_season) %>% summarize("peak_flu" = max(as.numeric(weekly_rate)))
#Select the seasons with the max and min peak flu
max_season <- compare_df %>% filter(peak_flu == max(peak_flu, na.rm = TRUE))
min_season  <- compare_df %>% filter(peak_flu == min(peak_flu, na.rm = TRUE))
#the minimum peak was in the 21-22 season with a value of 1.2 and the maximum 
#The maximum peak was in the 24-25 season with a value of 13.7. 
```

### 3.7 
* Add a column `overall_mean_flu_rate` to the `df15_25` data set that computes the mean weekly rate for each date. 
Hint: you will have to use the command `group_by` and group both by the `num_week` and the `virus_type_category` columns. 


```{r 3.7response}
df15_25 <- df15_25 %>% group_by(num_week, virus_type_category) %>% mutate("overall_mean_flu_rate" = mean(weekly_rate, na.rm=TRUE))

```

### 3.8

To understand the data better, we compute the difference between the 21-22 flu season and the overall mean hospitalization rate for each week. We then repeat this for the 24-25 flu season:

* Create a new column in the `df15_25` data frame called `diff_rate` obtained by subtracting the weekly rates  from the `overall_mean_flu_rate`. Remember to group by the `num_week` column as you want to compare each week in the data. Keep only the 21-22 and 24-25 seasons, by  storing the results in new data frames called `diff_min_season` and `diff_max_season` respectively.

```{r 3.8response}
diff_min_season <- df15_25 %>%  group_by(num_week) %>%
  mutate("diff_rate" = weekly_rate - overall_mean_flu_rate) %>% 
  filter(flu_season == "2021-22") 
diff_max_season <- df15_25 %>% group_by(num_week) %>%
  mutate("diff_rate" = weekly_rate - overall_mean_flu_rate) %>% 
  filter(flu_season == "2024-25") 
```
...
```{r 3.9response}
p38 <- ggplot(data=diff_max_season%>% select(diff_rate), aes(x=diff_rate)) +  geom_histogram(fill = "blue", alpha=0.2) + geom_histogram(data=diff_min_season%>% select(diff_rate), fill = "red", alpha=0.2) + theme(legend.location = "right")
p38
```
...
Hint: create twi variables by selecting the `weekly_rate` column from `df15_25` and filtering by the desired flu seasons. 

```{r 3.10response}
minSeason <-  df15_25 %>% filter(flu_season == "2021-22") %>% pull(weekly_rate)
maxSeason <-  df15_25 %>% filter(flu_season == "2024-25") %>% pull(weekly_rate)
mytest = t.test(minSeason, maxSeason)
mytest
...
This is a simple analysis using a t-test. For more sophisticated analysis, you can take a look at the
`tseries` package for analysing time series (https://cran.r-project.org/web/packages/tseries/index.html) or the fable` package for forecasting models (https://fable.tidyverts.org/).
