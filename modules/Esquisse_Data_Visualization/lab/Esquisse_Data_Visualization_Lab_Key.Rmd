...
This dataset has a lot of variables, so first run the below code to subset it so that you're only working with these variables: `CES4.0Percentile`,  `Asthma`, and `ChildrenPercLess10`. We will also categorize `CES4.0Percentile` into three categories (high, middle, and low) to make visualization a little easier!
...
Try creating a plot in `esquisse` using the `ces_sub` data.
...
Change the plot to a different type of plot. Copy and paste the code into the chunk below. Close Esquisse, and run the chunk below to generate a ggplot.
