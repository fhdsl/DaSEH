...
Use the `mutate()` function to create a new column named `date_formatted`, based on the `date_end` column. Hint: use `mdy()` function. Reassign to `covidww`.

`date_end`: This is confusingly named, but this variable actually tracks the date a given sample was taken. It is called `date_end` because the NWSS is reporting how much the amount of covid in the sample taken on that day has changed compared to what it was 15 days prior. The `date_end` means "the date that ends the 15 day comparison period".
```
# General format
...
```{r 1.7response}
covidww <- covidww %>% mutate(date_formatted = mdy(date_end))
```
...
```{r P.1response}
covidww <- covidww %>% relocate(date_formatted, .before = date_end)

# alternative
# covidww <- covidww %>% select(date_end, date_formatted, everything()) %>% head() 
