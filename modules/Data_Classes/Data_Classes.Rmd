...
```{r, results='hide'}
#library(dasehr)
covid_ww <- SARS_CoV_2_Wastewater_Data
```
...
```{r}
covid_ww_updated <- mutate(covid_ww, pop_int= as.integer(population_served))

covid_ww_updated %>% select(population_served, pop_int)
```
...
```{r}
covid_ww <- mutate(covid_ww, date_start_formatted = mdy(date_start)) %>% 
...
```{r}
covid_ww %>% mutate(year = year(date_start_formatted)) %>%
...
```{r,}
covid_ww_mat <- select(covid_ww, contains("Missouri")) %>%
  head(n = 3)
covid_ww_mat
as.matrix(covid_ww_mat)
```
