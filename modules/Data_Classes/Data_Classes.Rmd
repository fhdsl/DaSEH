...
Here's a dataset on the covid viral load measured in wastewater between 2022 and 2024, collected by the collected by the National Wastewater Surveillance System.

Let's look at the *first_sample_date* variable
* records the first time samples were collected at a site

```{r, results='hide'}
library(dasehr)
covidww <- covid_wastewater %>%
  select(contains("first_sample_date")) %>%
  sample_n(10)
...
```{r}
covidww %>% 
  mutate(sampling_start = mdy(first_sample_date))
```
...
*NOTE:* in the covid_wastewater dataset, "date_start" is always 15 days before "date_end"

```{r,}
covidww_mat <- select(covid_wastewater, contains("date")) %>%
  head(n = 3)
covidww_mat
