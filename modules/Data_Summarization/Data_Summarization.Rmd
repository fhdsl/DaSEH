...
We can use the `CO_heat_ER` object from the `dasehr` package to explore different ways of summarizing data. 
...
```{r}
CO_heat_ER_grouped <- CO_heat_ER %>% group_by(year)
CO_heat_ER_grouped
```
...
```{r}
CO_heat_ER %>% count(year)
CO_heat_ER %>% group_by(year) %>% summarize(n()) # n() typically used with summarize
```
...
```{r, message = FALSE}
CO_heat_ER_year <- CO_heat_ER %>% pull(year) # pull() to make a vector
CO_heat_ER_year %>% unique() # similar to distinct()
```
...
```{r, message = FALSE}
CO_heat_ER_year %>% unique() %>% length() # similar to n_distinct()
```
