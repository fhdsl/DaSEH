...
The `TotalPop` column includes information about the population for each census tract as of the 2010 census. (Remember, a census tract a small, relatively permanent area within a county used to present data from the census. Each row in the `ces` dataset corresponds to a single census tract.)

What was the population of California based on the 2010 census? (use `sum()` and the `TotalPop` column)
...
How many census tracts are there in California county?? Use `count()` on the column named `CaliforniaCounty`. Use `ces` as your input data.
...
Modify your code from question 2.1 to break down each county by ZIP code. Use `count()` on the columns named `CaliforniaCounty` and `ZIP`.
