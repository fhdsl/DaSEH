...
We will work with data called `ER` (number of shipments (count) of either oxycodone or hydrocodone pills (DOSAGE_UNIT)).
...
```{r}
renamed_ER<- rename(ER, County = BUYER_COUNTY)
head(renamed_ER)
```
...
```{r}
ER_lower<- rename_with(ER, tolower)
head(ER_lower, 3)
```
...
```{r}
select(ER_30, year, BUYER_COUNTY)
```
...
```{r}
head(ER_30, 2)
select(ER_30, starts_with("B"), year)
```
...
```{r}
select(ER_30, starts_with("B"), ends_with("r"))
...
```{r}
select(ER_30, starts_with(c("B", "D")))
...
```{r, eval = FALSE}
filter(ER_30, count > 10000, year == 2012)
```

```{r}
filter(ER_30, count > 10000 & year == 2012) # same result
```
...
```{r}
filter(ER_30,  BUYER_STATE %in% c("CO","NM","GA"))

filter(ER_30,  BUYER_STATE == "CO"| BUYER_STATE == "NM"| BUYER_STATE == "GA") #equivalent
```
...
```{r}
filter(ER_30, year %in% c(2012,2014), BUYER_STATE %in% c("GA","CO"))
...
```{r}
filter(ER_30, "BUYER_STATE" == "CO") # this will not work
```

## Be careful with column names and `filter`

```{r}
filter(ER_30, BUYER_STATE == "CO")# this works!
```
...
```{r}
select(filter(ER_30, year > 2012), BUYER_STATE)
```
...
```{r}
head(select(ER_30, year, BUYER_STATE), 2)
```

## Nesting can get confusing looking

```{r}
select(filter(ER_30, year > 2000 & BUYER_STATE == "CO"), year, count)
```
...
```{r}
ER_30_CO <- filter(ER_30, year > 2000 & BUYER_STATE == "CO")
ER_30_CO <- select(ER_30_CO, year, count)
...
```{r}
ER_30 %>% filter(year > 2000 & BUYER_STATE == "CO") %>% select(year, count)
```
...
```{r}
ER_30 <- mutate(ER_30, newcol = count * 2)
head(ER_30, 4)
```
...
```{r}
head(ER_30, 2)
ER_30 %>% select(year, count, BUYER_STATE, BUYER_COUNTY) %>%
head(2)
```
...
tb_carb <- relocate(ER_30, year, .before = BUYER_STATE)
...
```{r}
arrange(ER_30, count, desc(year)) %>% head(n = 2)

arrange(ER_30, desc(year), count) %>% head(n = 2)
```
...
colnames(ER_30)[1:3] <- c("County", "State", "Year") # reassigns
head(ER_30)
```
