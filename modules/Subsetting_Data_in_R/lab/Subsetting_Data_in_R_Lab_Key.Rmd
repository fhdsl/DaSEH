...
Pull the variable `Asthma` from `ces_sub`. How does this differ form selecting it? Use head() to take a look at both options.
...
Subset the rows of `ces2` that have **more** than 100 for `Asthma` - how many rows are there? Use `filter()`.
...
Subset the rows of `ces` that have an `Traffic` value **less** than 500 and an `Asthma` value **more** than 100  - how many are there?


```{r 2.5response}
filter(ces, Traffic < 500 & Asthma > 100) # all of these options work
nrow(filter(ces, Traffic < 500 & Asthma > 100))
nrow(filter(ces, Traffic < 500, Asthma > 100))
```

### 2.6

Subset the rows of `ces` that have an `Traffic` value **less than or equal to**  500 and an `Asthma` value **more** than 100  - how many are there?
...
Why don't the answers for 2.5 and 2.6 differ?
...
`CES4.0PercRange`: Percentile of the CalEnviroScreen score, grouped by 5% increments. The CalEnviroScreen score is a measure of the negative environmental effects seen in a given region.
