...
15\. How many possible "Year" and "Month" combinations are in `rainfall_long`? Use `count()`. How does this compare to `nrow()`? What can you conclude?
...
18\. Check the class of the "Month" column in each dataset. Reformat "Month" in `rainfall_long` so that it matches the format in `water_Ham`. Use `case_when()` and reassign to `rainfall_long`.
