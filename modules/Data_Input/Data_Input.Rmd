...
-  `>` Import Dataset
-  `>` From Text (`readr`)
-  `>` paste the url (https://daseh.org/data/CalEnvironmentalScreen_data.csv)
...
You see a new object called `CalEnvironmentalScreen_data` in your environment pane (top right). The table button opens the data for you to view.
...
library(readr)
dat <- read_csv(
  file = "https://daseh.org/data/CalEnvironmentalScreen_data.csv
"
)
...
# EQUIVALENT TO
dat <- read_csv(
  "https://daseh.org/data/CalEnvironmentalScreen_data.csv"
)
```
...
dat <- read_csv(file = "/Users/avahoffman/Downloads/CalEnvironmentalScreen_data.csv")

dat <- read_csv(file = "CalEnvironmentalScreen_data.csv")
```
...
If we add the CalEnvironmentalScreen_data.csv file to the R Project folder, we can use the file name for the `file` argument:
...
dat <- read_csv(file = "data/CalEnvironmentalScreen_data.csv")
```
