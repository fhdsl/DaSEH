...
-  `>` Import Dataset
-  `>` From Text (`readr`)
-  `>` paste the url (https://daseh.org/data/CO_ER_heat_visit.csv)
...
You see a new object called `CO_ER_heat_visit` in your environment pane (top right). The table button opens the data for you to view.
...
library(readr)
dat <- read_csv(
  file = "https://daseh.org/data/CO_ER_heat_visit.csv"
)
...
# EQUIVALENT TO
dat <- read_csv(
  "https://daseh.org/data/CO_ER_heat_visit.csv"
)
```
...
dat <- read_csv(file = "/Users/avahoffman/Downloads/CO_ER_heat_visit.csv")

dat <- read_csv(file = "CO_ER_heat_visit.csv")
```
...
**R Projects** can make it easier to find files. Check out [this resource](https://daseh.org/resources/R_Projects.html).
