...
```{r P.3response}
ces_Alameda <- read_excel(path = "ces_Alameda.xlsx")
```
...
```{r}
test <- read_delim("https://daseh.org/data/CalEnviroScreen_data_AlamedaCounty.xlsx", delim = "\t")
test
```

```{r P.4response}
# It should be a red flag to see that there is only one column 
