...
```{r, message = FALSE}
library(dasehr)
wide <- CO_heat_ER_wide
head(wide)
```

## Mission: Taking the average visit rates by location

Let's imagine we want to create a table of visit rate by county and year. Results should look something like:

```{r, message = FALSE, echo = FALSE}
example <- tibble(county = c("Boulder", "Boulder", "Denver", "Denver"),
                  year = c("2011", "2012", "2011", "2012"),
                  visit_rate = c("4", "5", "3", "3"))
example
```
...
```{r}
wide %>% select(location_type) %>% head()
long %>% select(location_type) %>% head()
```

## Cleaning up long data

We will use `str_replace` from the `stringr` package to put `_county` in the county names 

```{r}
long <- long %>% mutate(
  county = str_replace(string = county, pattern = "Boulder", replacement = "Boulder_county"),
  county = str_replace(string = county, pattern = "Denver", replacement = "Denver_county")
)
long
```
...
```{r}
long <- long %>% 
  separate(county, into = c("location_name", "location_type"), sep = "_")
long
```
...
long %>% 
  ...
  summarize("avg_visit_rate" = mean(visit_rate, na.rm = TRUE))
```
...
```{r}
wide <- long %>% pivot_wider(names_from = "year", 
                             values_from = "visit_rate") 
wide
```
