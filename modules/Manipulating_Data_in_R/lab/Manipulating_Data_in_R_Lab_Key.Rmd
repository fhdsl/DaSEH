...
Use `pivot_wider` to reshape "co2_long". Use "country" for the `names_from` argument. Use "value" for the `values_from` argument. Call this new data `co2_wide`. 
...
```
# General format
anti_join(data1, data2, by = "") |> select(columnname) |> distinct()
```

```{r 2.4response}
anti_join(co2, cc, by = "country") |> select(country) |> distinct()
