...
co2_compare2 <-
  co2 <- yearly_co2_emissions %>%
  rename(CO2_2011 = `1960`) %>%
  rename(CO2_2012 = `1970`) %>%
  rename(CO2_2013 = `1980`) %>%
  rename(CO2_2014 = `1990`) %>%
  select(country, starts_with("CO2_")) %>%
  pivot_longer(cols = !country) %>%
...
```{r 2.3response}
co2_long <- co2_long %>% mutate(name = recode(name, CO2_2011 = '2011', CO2_2012 = '2012', CO2_2013 = '2013', CO2_2014 = '2014'))

#merged <- inner_join(CO_heat, co2_long)
#merged <- inner_join(CO_heat, co2_long, by = c("year" = "name"))
#nrow(merged)
```
...
```{r 2.3responsePt2}
co2_long <- co2_long %>% mutate(name = as.double(name))
...
```{r 2.4response}
full <- full_join(CO_heat, co2_long, by = c("year" = "name"))
nrow(full)
```
...
```{r P.3response}
left <- left_join(CO_heat, co2_long, by = c("year" = "name"))
nrow(left)
```
...
```{r P.4response}
right <- right_join(CO_heat, co2_long, by = c("year" = "name"))
nrow(right)
```
...
```{r P.5response}
in_CO_heat_only <- anti_join(CO_heat, co2_long, by = c("year" = "name")) %>% select(year)
in_co2_long_only <- anti_join(co2_long, CO_heat, by = c("name" = "year")) %>% select(name)
in_CO_heat_only
in_co2_long_only
```
