...
er_visits_all <- er_visits_by_CO_county

ggplot(er_visits_all, aes(x = year, y = rate, color = county)) +
  geom_point() +
  geom_line()
```
...
```{r, fig.width=5, fig.height=3, fig.align='center'}
er_visits_all %>% ggplot(aes(x = year,
                      y = rate,
                      color = county)) +
  geom_line(size = 0.8)
```
...
Let's work with two slightly different versions of the CO ER visits dataset. We used the first one, `er_visits_all`, a little earlier in this lecture. This dataset includes county-level data on the number of visits to the ER for heat-related illness, as well as the age-adjusted rate and the 95% confidence bounds. The dataset includes:
...
Let's start with `er_visits_all`.

```{r, fig.width=5, fig.height=3, fig.align='center'}
ggplot(er_visits_all, aes(x = year,
                 y = rate)) +
  geom_line()
```
...
But what if we wanted to look at rates by county for different genders? Let's work a little with the `CO_heat_ER_bygender` dataset. This is a subset of the larger CO ER visits dataset.
...
ggplot(er_visits_gender, aes(x = year,
                 y = rate,
                 color = county)) +
  geom_line() +
  facet_grid( ~ gender)+
...
rp_fac_plot <- ggplot(er_visits_gender, aes(x = year, y = rate,color = county)) +
                  geom_line()  +
                  geom_point() +
