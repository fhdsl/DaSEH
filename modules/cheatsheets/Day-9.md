...
| `dplyr`| [`across()`](https://www.rdocumentation.org/packages/dplyr/versions/1.0.10/topics/across)| `iris |>` <br> `group_by(Species) |>` <br>  `summarise(across(starts_with("Sepal"), ~ mean(.x, na.rm = TRUE)))` <br> or `iris |> mutate(across(contains("Sepal"), round))`| Apply a function across groups or columns in a dataframe.
